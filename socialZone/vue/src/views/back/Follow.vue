...
const save = () => {
  ...
  request.post("/follow", form.value).then(res => {
    if (res.code === '200') {
    ...
    <el-card>
      <el-table :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="关注" align="center">
          <template #default="scope">
            <span v-if="scope.row.itemId">{{ users.find(item => item.id === scope.row.itemId)?.nickname }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogFormVisible" :title="form.id ? '编辑' : '新增'" width="30%" center>
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="关注">
          <el-select v-model="form.itemId" placeholder="请选择关注" style="width: 220px">
            <el-option v-for="item in users" :key="item.id" :label="item.nickname" :value="item.id" />
          </el-select>
        </el-form-item>
